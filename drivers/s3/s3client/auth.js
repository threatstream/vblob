...
exports.stringToSign = function(options) {
    ...
    return [
        options.verb, options.md5, options.contentType, options.date //.toUTCString()
...
exports.canonicalizeResource = function(resource) {
    ...
    Object.keys(urlObj.query).forEach(function(qs) {
        if (['acl', 'location', 'logging', 'notification', 'partNumber', 'policy',
            ...
            qbuf.push(qs + (urlObj.query[qs] !== '' ? '=' + /*encodeURIComponent*/ (
                urlObj.query[qs]) : ''));
        }
    });
