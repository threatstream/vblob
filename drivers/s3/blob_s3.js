        ...
        if (char_buf !== "") {
            ...
            if (cur_obj[name]) {
                if (cur_obj[name].push) {
                    ...
                    cur_obj[name] = [cur_obj[name], char_buf];
                }
            }
            else {
                cur_obj[name] = char_buf;
            }
        }
        else {
            ...
            if (cur_obj2[name]) {
                if (cur_obj2[name].push) {
                    ...
                    cur_obj2[name] = [cur_obj2[name], cur_obj];
                }
            }
            else {
                cur_obj2[name] = cur_obj;
            }
            cur_obj = cur_obj2;
        }
        char_buf = "";
...
function general_handler(cl, req, callback, head) {
    ...
    req.on('response', function(res) {
        ...
        if (res.statusCode === 307) {
            ...
            var span = 60;
            cl.region_cache[sp[0]] = {
                "name": (sp[1] + '.' + sp[2] + '.' + sp[3]),
                "expire": (new Date().valueOf() + span * 1000)
            };
        }
...
S3_blob.prototype.container_delete = function(bucket_name, callback) {
    var req = this.client.del({
        bucket: bucket_name,
        endpoint: this.region_cache[bucket_name]
    }, {});
    general_handler(this, req, callback);
    req.end();
};

S3_blob.prototype.file_create = function(bucket_name, filename, create_options, create_meta_data,
    data, callback) {
    var resp_header, resp_code, resp_body;
    ...
    this.client.putStream2(data, {
        bucket: bucket_name,
        "filename": filename,
        endpoint: this.region_cache[bucket_name]
    }, header, function(err, res, chunk) {
        ...
        {
            if (!parser) {
                throw "Create_File response error!";
            } //shouldn't happen!
            else {
                parser.write(chunk);
            }
...
S3_blob.prototype.file_copy = function(bucket_name, object_key, header, header2, callback) {
    ...
    for (var idx = 0; idx < keys.length; idx++) {
        if (keys[idx].match(/^x-amz-acl$/i)) {
            delete header[keys[idx]];
            break;
        } //remove acl, always create private object
    }
...
function check_client(client, callback) {
    ...
    var resp_body = {
        Code: 500,
        Message: "No network connection to S3"
    };
    callback(resp_code, resp_header, resp_body, null);
    return false;
}
...
S3_Driver.prototype.file_create = function(bucket_name, object_key, options, metadata, data_stream,
    callback) {
    if (check_client(this.client, callback) === false) return;
    this.client.file_create(bucket_name, object_key, options, metadata, data_stream, callback);
};

S3_Driver.prototype.file_copy = function(bucket_name, object_key, source_bucket, source_object_key,
