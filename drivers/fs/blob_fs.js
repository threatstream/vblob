...
function start_collector(option, fb) {
    ...
    var ec_interval;
    try {
        if (isNaN(ec_interval = parseInt(option.ec_interval, 10))) throw 'isNaN';
    }
    catch (err) {
        ec_interval = 1500;
    }
    ...
    fb.ecid = setInterval(function() {
        ...
        exec(node_exepath + " " + ec_exepath + " " + fb.root_path + " --tmp " + fb.tmp_path +
            " > /dev/null",
...
function start_gc(option, fb) {
    ...
    var gctmp_interval;
    try {
        if (isNaN(gc_interval = parseInt(option.gc_interval, 10))) throw 'isNaN';
    }
    catch (err) {
        gc_interval = 600000;
    }
    try {
        if (isNaN(gcfc_interval = parseInt(option.gcfc_interval, 10))) throw 'isNaN';
    }
    catch (err) {
        gcfc_interval = 1500;
    }
    try {
        if (isNaN(gctmp_interval = parseInt(option.gctmp_interval, 10))) throw 'isNaN';
    }
    catch (err) {
        gctmp_interval = 3600000;
    }
    ...
    fb.gcid = setInterval(function() {
        ...
        gc_status = 1;
        exec(node_exepath + " " + gc_exepath + " " + fb.root_path + " --tmp " + tmp_path +
            " > /dev/null",
            function(error, stdout, stderr) {
    ...
    fb.gcfcid = setInterval(function() {
        if (gcfc_status === 1 || gc_hash === null || Object.keys(gc_hash).length === 0)
            return; //optimization to avoid empty loop
        gcfc_status = 1;
        var tmp_fn = tmp_path + "/gcfc-" + new Date().valueOf() + "-" + Math.floor(Math.random() *
        ...
        fs.writeFile(tmp_fn, JSON.stringify(tmp_hash), function(err) {
            ...
            exec(node_exepath + " " + gcfc_exepath + " " + tmp_fn + " " + fb.root_path +
                " --tmp " + tmp_path + " > /dev/null",
                function(error, stdout, stderr) {
    ...
    setTimeout(function() {
        ...
        exec(node_exepath + " " + gctmp_exepath + " " + fb.root_path + " --ts " +
            current_ts + " > /dev/null",
            function(error, stdout, stderr) {
                if (error || stderr) {
                    fb.logger.warn('error in gc left over tmp files: ' + error ? error :
                        '' + '-- ' + stderr ? stderr : '');
                }
                fb.logger.info(
                    'left over tmp files collected; now start to collect left over gc files'
                );
                exec(node_exepath + " " + gc_exepath + " " + fb.root_path + " --ts " +
                    current_ts + " ",
                    function(error2, stdout2, stderr2) {
                        if (error2 || stderr2) {
                            fb.logger.warn('error in gc left over gc files: ' +
                                error2 ? error2 : '' + '-- ' + stderr2 ?
                                stderr2 : '');
                        }
...
function start_quota_gathering(fb) {
    fs.readdir(fb.root_path, function(err, dirs) {
        ...
        evt.on("Get Usage", function(dir_name, idx) {
            fs.readFile(fb.root_path + "/" + dir_name + "/~enum/quota", function(
                err, data) {
                if (err) {
                    obj_count[idx] = null;
                    used_quota[idx] = null;
                    ...
                    try {
                        ...
                        obj_count[idx] = parseInt(obj.count, 10);
                        used_quota[idx] = parseInt(obj.storage, 10);
                    }
                    catch (e) {
                        obj_count[idx] = null;
                        used_quota[idx] = null;
                ...
                counter--;
                if (counter === 0) {
                    evt.emit("Start Aggregate");
                }
            });
        });
        evt.on("Start Aggregate", function() {
            for (var i = 0; i < dirs.length; i++) {
                if (used_quota[i] === null) {
                    continue;
                }
                sum += used_quota[i];
                sum2 += obj_count[i];
...
function FS_blob(option, callback) //fow now no encryption for fs
{
    ...
    if (option.quota) {
        this.quota = parseInt(option.quota, 10);
        this.used_quota = 0;
    }
    else {
        this.quota = 100 * 1024 * 1024;
        this.used_quota = 0;
    } //default 100MB
    if (option.obj_limit) {
        this.obj_limit = parseInt(option.obj_limit, 10);
        this.obj_count = 0;
    }
    else {
        this.obj_limit = 10000;
        this.obj_count = 0;
    ...
    fs.stat(this1.root_path, function(err, stats) {
        if (!err) {
            ...
            this1.logger.error(('root folder in fs driver is not mounted'));
        }
        if (callback) {
            callback(this1, err);
        }
...
FS_blob.prototype.container_create = function(container_name, callback, fb) {
    ...
    fs.stat(fb.root_path + "/" + container_name + "/ts", function(err, stats) {
        ...
        try {
            ...
            {
                fb.logger.debug(("timestamp " + c_path +
                    "/ts does not exist. Need to create one"));
            ...
            var resp = {};
            error_msg(500, "InternalError", "Cannot create bucket because: " + err1,
                resp);
...
FS_blob.prototype.container_delete = function(container_name, callback, fb) {
    ...
    if (fs.existsSync(c_path) === false) { //shortcut, remove directly
    ...
    fn1 = fb.tmp_path + '/find1-' + da + "-" + Math.floor(Math.random() * 10000) + "-" + Math.floor(
        Math.random() * 10000);
    fn2 = fb.tmp_path + '/find2-' + da + "-" + Math.floor(Math.random() * 10000) + "-" + Math.floor(
        Math.random() * 10000);
    var child1 = exec('find ' + c_path + "/*/* -type d -empty > " + fn1, function(error, stdout,
        stderr) {
        var child2 = exec('find ' + c_path + "/*/* -type d > " + fn2, function(error,
            stdout, stderr) {
            var child3 = exec('diff -q ' + fn1 + " " + fn2, function(error,
                stdout, stderr) {
                if (stdout === null || stdout === undefined || stdout ===
                    '') {
                    var child = exec('rm -rf ' + fb.root_path + "/" +
                        container_name,
                        function(error, stdout, stderr) {
                            var header = common_header();
                            resp_code = 204;
                            resp_header = header;
                            callback(resp_code, resp_header, null,
                                null);
                        }
                    );
                }
...
function get_key_fingerprint(filename) {
    ...
    var prefix, suffix;
    var file2 = filename.replace(
        /(\+|=|\^|#|\{|\}|\(|\)|\[|\]|%|\||,|:|!|;|\/|\$|&|@|\*|`|'|"|<|>|\?|\\)/g, "_"); //replacing all special chars with "_"
    if (file2.length < 8) {
...
function generate_version_id(key) {
    var da = new Date().valueOf();
    return key + '-' + da + '-' + Math.floor(Math.random() * 1000) + '-' + Math.floor(Math.random() *
        1000);
}
...
FS_blob.prototype.file_create = function(container_name, filename, create_options, create_meta_data,
    data, callback, fb) {
    var resp = {};
    ...
    //QUOTA
    if (this.quota && this.used_quota + parseInt(create_meta_data["content-length"], 10) > this
        .quota || this.obj_limit && this.obj_count >= this.obj_limit) {
    ...
    var prefix1 = key_fingerprint.substr(0, PREFIX_LENGTH),
        prefix2 = key_fingerprint.substr(PREFIX_LENGTH, PREFIX_LENGTH);
    ...
    var meta_json = {
        vblob_file_name: filename,
        vblob_file_path: "blob/" + prefix_path + version_id
    };
    try {
    ...
    if (!create_prefix_folders([c_path + "/blob", prefix1, prefix2], callback)) {
        fs.unlink(temp_path, function(err) {});
        return;
    }
    if (!create_prefix_folders([c_path + "/versions", prefix1, prefix2], callback)) {
        fs.unlink(temp_path, function(err) {});
        return;
    }
    if (!create_prefix_folders([c_path + "/meta", prefix1, prefix2], callback)) {
        fs.unlink(temp_path, function(err) {});
        return;
    }
    //step 4 stream blob
    ...
    data.on("data", function(chunk) {
        if (file_size < 512000 || !openssl_available) md5_etag.update(chunk);
        else md5_etag = null;
    ...
    var closure1 = function(md5_etag) {
        var opts = {
            vblob_file_name: filename,
            vblob_file_path: "blob/" + prefix_path + version_id,
            vblob_file_etag: md5_etag,
            vblob_file_size: file_size,
            vblob_file_version: version_id,
            vblob_file_fingerprint: key_fingerprint
        };
        if (create_options['content-md5']) {
            ...
            {
                if (resp !== null) {
                    error_msg(400, "InvalidDigest",
                        "The Content-MD5 you specified was invalid.", resp);
                    callback(resp.resp_code, resp.resp_header, resp.resp_body, null);
                }
                fb.logger.error((filename + ' md5 not match: uploaded: ' + md5_base64 +
                    ' specified: ' + create_options['content-md5']));
                data.destroy();
    ...
    stream.once("close", function() {
        ...
        if (md5_etag) {
            ...
            var child = exec('openssl md5 ' + blob_path,
                function(error, stdout, stderr) {
                    if (error) {
                        fb.logger.error(blob_path + ' md5 calculation error: ' +
                            error);
                        error_msg(500, "InternalError", "Error in md5 calculation:" +
                            error, resp);
                        callback(resp.resp_code, resp.resp_header, resp.resp_body,
                            null);
                        data.destroy();
                        remove_uploaded_file(blob_path);
                        return;
...
FS_blob.prototype.file_create_meta = function(container_name, filename, temp_path, opt, callback,
    fb, is_copy) {
    var resp = {};
    var resp_code, resp_header, resp_body;
    resp_code = resp_header = resp_body = null;
    ...
    fs.writeFile(temp_path, JSON.stringify(doc), function(err) {
        if (err) {
            fb.logger.error(("In creating file " + filename +
                " meta in container_name " + container_name + " " + err));
            if (resp !== null) {
                error_msg(404, "NoSuchBucket", err, resp);
                callback(resp.resp_code, resp.resp_header, resp.resp_body, null);
            }
            fs.unlink(temp_path, function(err) {});
            fs.unlink(fb.root_path + "/" + container_name + "/" + doc.vblob_file_path,
                function(err) {});
            return;
        }
        fb.logger.debug(("Created meta for file " + filename + " in container_name " +
            container_name));
        var header = common_header();
        ...
        if (is_copy) {
            resp.resp_body = {
                "CopyObjectResult": {
                    "LastModified": new Date(doc.vblob_update_time).toISOString(),
                    "ETag": '"' + opt.vblob_file_etag + '"'
                }
            };
        ...
        //step 5.6 add to /~GC
        fs.symlink(temp_path, fb.root_path + "/" + container_name + "/" + GC_FOLDER +
            "/" + doc.vblob_file_version,
            function(err) {
                if (err) {
                    fb.logger.error(("In creating file " + filename +
                        " meta in container_name " + container_name + " " +
                        err));
                    if (resp !== null) {
                        error_msg(500, "InternalError", err, resp);
                        callback(resp.resp_code, resp.resp_header, resp.resp_body,
                ...
                if (!gc_hash[container_name][doc.vblob_file_fingerprint]) gc_hash[
                    container_name][doc.vblob_file_fingerprint] = {
                    ver: [doc.vblob_file_version],
                    fn: doc.vblob_file_name
                };
                else gc_hash[container_name][doc.vblob_file_fingerprint].ver.push(
                    doc.vblob_file_version);
                //step 6 mv to versions
                var prefix1 = doc.vblob_file_version.substr(0, PREFIX_LENGTH),
                    prefix2 = doc.vblob_file_version.substr(PREFIX_LENGTH,
                        PREFIX_LENGTH);
                //link to version, so version link > 1, gc won't remove it at this point
                fs.link(temp_path, fb.root_path + "/" + container_name +
                    "/versions/" + prefix1 + "/" + prefix2 + "/" + doc.vblob_file_version,
                    function(err) {
                        if (err) {
                            fb.logger.error(("In creating file " + filename +
                                " meta in container_name " +
                                container_name + " " + err));
                            if (resp !== null) {
                                error_msg(500, "InternalError", err, resp);
                                callback(resp.resp_code, resp.resp_header, resp
                                    .resp_body, null);
                            }
                            return;
                        }
                        //step 7 ln -f meta/key versions/version_id
                        var child = exec('ln -f ' + fb.root_path + "/" +
                            container_name + "/versions/" + prefix1 + "/" +
                            prefix2 + "/" + doc.vblob_file_version + " " +
                            fb.root_path + "/" + container_name + "/meta/" +
                            prefix1 + "/" + prefix2 + "/" + doc.vblob_file_fingerprint,
                            function(error, stdout, stderr) {
                                //now we can remove temp
                                fs.unlink(temp_path, function(err) {});
                                //step 8 respond
                                fb.logger.debug("file creation " + doc.vblob_file_version +
                                    " complete, now reply back...");
                                callback(resp.resp_code, resp.resp_header,
                                    resp.resp_body, null);
                            }
                        );
                    });
...
FS_blob.prototype.file_delete_meta = function(container_name, filename, callback, fb) {
    ...
    fs.symlink(c_path + "/" + TEMP_FOLDER + "/" + version_id, c_path + "/" + GC_FOLDER + "/" +
        version_id,
        function(err) {
            if (err) {
                var resp = {};
            ...
            //add to gc cache
            if (!gc_hash[container_name]) gc_hash[container_name] = {};
            if (!gc_hash[container_name][key_fingerprint]) gc_hash[container_name][
                key_fingerprint
            ] = {
                ver: [version_id],
                fn: filename
            };
            else gc_hash[container_name][key_fingerprint].ver.push(version_id);
...
FS_blob.prototype.file_copy = function(container_name, filename, source_container, source_file,
    options, metadata, callback, fb, retry_cnt) {
    var resp = {};
    //step 1 check container existence
    var c_path = this.root_path + "/" + container_name;
    ...
    var prefix1 = key_fingerprint.substr(0, PREFIX_LENGTH),
        prefix2 = key_fingerprint.substr(PREFIX_LENGTH, PREFIX_LENGTH);
    var src_prefix1 = src_key_fingerprint.substr(0, PREFIX_LENGTH),
        src_prefix2 = src_key_fingerprint.substr(PREFIX_LENGTH, PREFIX_LENGTH);
    var prefix_path = prefix1 + "/" + prefix2 + "/";
    ...
    var etag_match = null,
        etag_none_match = null,
        date_modified = null,
        date_unmodified = null;
    var meta_dir = null;
    if (true) {
    ...
    fs.readFile(src_meta_path, function(err, data) {
        if (err) {
            ...
            if (retry_cnt < MAX_COPY_RETRY) { //reduce the false negative rate
                setTimeout(function(fb1) {
                    fb1.file_copy(container_name, filename, source_container,
                        source_file, options, metadata, callback, fb1,
                        retry_cnt + 1);
                }, Math.floor(Math.random() * 1000) + 100, fb);
                return;
            }
        ...
        if (true) {
            ...
            if ((etag_match && obj.vblob_file_etag !== etag_match) ||
                ...
                error_msg(412, "PreconditionFailed",
            ...
            if (obj["content-disposition"]) dest_obj["content-disposition"] = obj[
                "content-disposition"];
            if (obj["content-encoding"]) dest_obj["content-encoding"] = obj[
                "content-encoding"];
            ...
            if (meta_dir === 'COPY') {
                if (source_container === container_name && source_file === filename) {
                    error_msg(400, "NotImplemented", "The headers are not supported",
                        resp);
                    callback(resp.resp_code, resp.resp_header, resp.resp_body, null);
                    return;
                }
                ...
                for (idx = 0; idx < keys.length; idx++) {
                    ...
                    else if (!key.match(/^content-length/i)) dest_obj[key] = metadata[
                        key];
                }
            }
            ...
            if (!create_prefix_folders([c_path + "/blob", prefix1, prefix2], callback))
                return;
            if (!create_prefix_folders([c_path + "/versions", prefix1, prefix2],
                    callback)) return;
            if (!create_prefix_folders([c_path + "/meta", prefix1, prefix2], callback))
                return;
            fs.writeFile(temp_path, JSON.stringify(dest_obj), function(err) {
                ...
                fs.link(src_path + "/" + obj.vblob_file_path, c_path + "/" +
                    dest_obj.vblob_file_path,
                    function(err) {
                        if (err) {
                            remove_uploaded_file(temp_path);
                            setTimeout(function(fb1) {
                                    fb1.file_copy(container_name,
                                        filename, source_container,
                                        source_file, options,
                                        metadata, callback, fb1);
                                }, Math.floor(Math.random() * 1000) +
                                100, fb);
                            return;
                        }
                        //ready to call file_create_meta
                        fb.file_create_meta(container_name, filename,
                            temp_path, dest_obj, callback, fb, true);
                    });
            });
        };
    });
...
FS_blob.prototype.file_read = function(container_name, filename, options, callback, fb, retry_cnt) {
    ...
    var key_fingerprint = get_key_fingerprint(filename);
    //step2.2 gen unique version id
    var file_path = c_path + "/meta/" + key_fingerprint.substr(0, PREFIX_LENGTH) + "/" +
        key_fingerprint.substr(PREFIX_LENGTH, PREFIX_LENGTH) + "/" + key_fingerprint; //complete representation: /container_name/filename
    ...
    fs.readFile(file_path, function(err, data) {
        if (err) {
            ...
            if (retry_cnt < MAX_READ_RETRY) {
                setTimeout(function(fb1) {
                    fb1.file_read(container_name, filename, options, callback,
                        fb1, retry_cnt + 1);
                }, Math.floor(Math.random() * 1000) + 100, fb);
                return;
            }
        ...
        if (modified_since === false ||
            ...
            resp.resp_header.etag = obj.vblob_file_etag;
            resp.resp_header["last-modified"] = obj.vblob_update_time;
            callback(resp.resp_code, resp.resp_header, /*resp.resp_body*/ null, null); //304 should not have body
            return;
        }
        header["content-type"] = obj["content-type"] ? obj["content-type"] :
            "binary/octet-stream";
        ...
        if (range !== null && range !== undefined) {
            header["Content-Range"] = "bytes " + (range.start !== undefined ? range.start :
                    "") + '-' + (range.end !== undefined ? range.end.toString() : "") +
                "/" + obj.vblob_file_size.toString();
            if (range.start === undefined) {
                range.start = obj.vblob_file_size - range.end;
                delete range.end;
            }
            if (range.end === undefined) {
                range.end = obj.vblob_file_size - 1;
            }
            ...
            if (verb === "get") { //TODO: retry for range read?
                if (range.start < 0 || range.start > range.end ||
                    range.start > obj.vblob_file_size - 1 || range.end > obj.vblob_file_size -
                    1) {
                    error_msg(416, 'InvalidRange',
                        'The requested range is not satisfiable', resp);
                    callback(resp.resp_code, resp.resp_header, resp.resp_body, null);
                    return;
                }
                ...
                st.on('error', function(err) {
                    st = null;
                    error_msg(503, 'SlowDown',
                        'The object is being updated too frequently, try later',
                        resp);
                    callback(resp.resp_code, resp.resp_header, resp.resp_body,
                        null);
                });
            ...
            if (verb === "get") {
                ...
                st.on('error', function(err) { //RETRY??
                    st = null;
                    fb.logger.error(("file " + obj.vblob_file_version +
                        " is purged by gc already!"));
                    //error_msg(508,'SlowDown','The object is being updated too frequently, try later',resp);
                    //callback(resp.resp_code, resp.resp_header, resp.resp_body, null);
                    setTimeout(function(fb1) {
                        fb1.file_read(container_name, filename, options,
                            callback, fb1);
                    }, Math.floor(Math.random() * 1000) + 100, fb);
                });
                st.on('open', function(fd) {
...
function query_files(container_name, options, callback, fb) {
    ...
    if (options.prefix) { //end of prefix range
        ...
        st2 = st2.substr(0, st2.length - 1) + String.fromCharCode(st2.charCodeAt(st2.length - 1) +
            1);
        low = idx;
        high = keys.length - 1;
        while (low <= high) {
    ...
    var limit1;
    try {
        limit1 = options["max-keys"] ? parseInt(options["max-keys"], 10) : 1000;
    }
    catch (err) {
        limit1 = 1000;
    }
    ...
    resp.resp_header = common_header();
    resp.resp_body = {
        "ListBucketResult": res_json
    };
    res_json = null;
    res_contents = null;
    keys = null;
    res_common_prefixes = null;
    callback(resp.resp_code, resp.resp_header, resp.resp_body, null);
}
...
FS_blob.prototype.file_list = function(container_name, options, callback, fb) {
    ...
    if (!enum_cache[container_name] || !enum_expire[container_name] || enum_expire[
            container_name] < now) {
        enum_cache[container_name] = null;
        try {
            enum_cache[container_name] = {
                tbl: JSON.parse(fs.readFileSync(fb.root_path + "/" + container_name + "/" +
                    ENUM_FOLDER + "/base"))
            };
...
function render_containers(dirs, callback, fb) {
    ...
    evt.on("Get Date", function(dir_name, idx) {
        fs.stat(fb.root_path + "/" + dir_name + "/ts", function(err, stats) {
            if (err) dates[idx] = null;
            else
                dates[idx] = stats.ctime;
            counter--;
            if (counter === 0) {
                evt.emit("Start Render");
            }
        });
    });
    evt.on("Start Render", function() {
        ...
        for (var i = 0; i < dirs.length; i++) {
            ...
            resp_body.ListAllMyBucketsResult.Buckets.Bucket.push({
                Name: dirs[i],
                CreationDate: new Date(dates[i]).toISOString()
            });
        }
        callback(resp_code, resp_header, resp_body, null);
    });
...
FS_Driver.prototype.file_read = function(container_name, file_key, options, callback) {
    ...
    if (options.range) {
        ...
        if (m[1] === m[2] && m[1] === '') {
            ...
            range1 = {};
            if (m[1] !== '') {
                range1.start = parseInt(m[1], 10);
            }
            if (m[2] !== '') {
                range1.end = parseInt(m[2], 10);
            }
        }
...
FS_Driver.prototype.file_create = function(container_name, file_key, options, metadata, data_stream,
    callback) {
    if (check_client(this.client, callback) === false) return;
    this.client.file_create(container_name, file_key, options, metadata, data_stream, callback,
        this.client);
};

FS_Driver.prototype.file_copy = function(container_name, file_key, source_container,
    source_file_key, options, metadata, callback) {
    if (check_client(this.client, callback) === false) return;
    this.client.file_copy(container_name, file_key, source_container, source_file_key, options,
        metadata, callback, this.client);
};
