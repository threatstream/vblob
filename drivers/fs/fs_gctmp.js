...
buck.on('gc', function(buck_idx) {
    try {
        ...
        evt.on('next', function(idx) {
            ...
            var fdir_path = root_path + "/" + evt.Container + "/blob/" + prefix1 +
                "/" + prefix2;
            fs.stat(trash_dir + "/" + filename, function(err, stats) {
                ...
                if (!force &&
                    (gc_timestamp && gc_timestamp < mtime //created later than the specified timestamp
                        || !gc_timestamp && current_ts < mtime +
                        MAX_TIMEOUT)) {
                    evt.Counter++;
                    evt.Batch--; //still within a valid window
                    if (evt.Batch === 0) {
                        evt.Batch = BATCH_NUM;
                        evt.emit('nextbatch');
                    }
                ...
                if (stats.nlink <= 2) fs.unlink(fdir_path + "/" + filename,
                    function() {}); //not an active version
