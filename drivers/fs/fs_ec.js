...
buck.on('compact', function(buck_idx) {
    try {
        ...
        var temp_file = tmp_path + "/" + new Date().valueOf() + "-" + Math.floor(Math.random() *
                ...
                if (!error) {
                    ...
                    evt2.on('next', function(idx2) {
                        ...
                        fs.readFile(file1, function(err2, data) {
                            if (!err2) {
                                ...
                                for (var i = 0; i < keys.length; i++) {
                                    ...
                                    if (obj[key] == 0) {
                                        if (ivt_enum[key]) key =
                                            ivt_enum[key];
                                        else continue; //look up file name by fingerprint
                                    }
                                    //CALC FINGERPRINT AND READ META
                                    var filename = get_key_fingerprint(
                                        key);
                                    ivt_enum[filename] = key;
                                    var pref1 = filename.substr(0, 2),
                                        pref2 = filename.substr(2, 2);
                                    try {
                                        ...
                                        var old_size = enum_base[key] ?
                                            enum_base[key].size : 0;
                                        enum_base[key] = {};
                                        enum_base[key].size = obj2.vblob_file_size;
                                        enum_base[key].etag = obj2.vblob_file_etag;
                                        enum_base[key].lastmodified =
                                            obj2.vblob_update_time;
                            ...
                            while (sync_cnt < MAX_TRIES) {
                                try {
                                    fs.writeFileSync(enum_dir + "/base",
                                        JSON.stringify(enum_base));
                                }
                                catch (e) {};
                                sync_cnt++;
                            }
                            ...
                            if (evt2.counter > 0) evt2.emit('next',
                                idx2 + 1);
                        });
                    }); //end of next
                    evt2.emit('next', 0);
