...
buck.on('gc', function(buck_idx) {
    try {
        ...
        for (var nIdx1 = 0; nIdx1 < trashes.length; nIdx1++) {
            ...
            try {
                var prefix1 = fileversion.substr(0, PREFIX_LENGTH),
                    prefix2 = fileversion.substr(PREFIX_LENGTH, PREFIX_LENGTH);
                var ver_path = root_path + "/" + containers[buck_idx] + "/versions/" +
                    prefix1 + "/" + prefix2 + "/" + fileversion;
        ...
        var enum_delta_file = enum_dir + "/delta-" + new Date().valueOf() + "-" + Math.floor(
            Math.random() * 10000) + "-" + Math.floor(Math.random() * 10000);
        ...
        evt.on('next', function(idx) {
            ...
            if (gc_timestamp) { //specified timestamp, check stats here
                var stats = null;
                try {
                    stats = fs.lstatSync(trash_dir + "/" + filename);
                }
                catch (err) {}
            ...
            var prefix1 = filename.substr(0, PREFIX_LENGTH),
                prefix2 = filename.substr(PREFIX_LENGTH, PREFIX_LENGTH);
            var fdir_path = root_path + "/" + evt.Container + "/versions/" +
                prefix1 + "/" + prefix2;
            var temp_file = tmp_path + "/gctmp-" + new Date().valueOf() + "-" +
                Math.floor(Math.random() * 10000) + "-" + Math.floor(Math.random() *
                    10000);
            var child = exec('find ' + fdir_path + "/ -type f -name \"" + filename +
                "-*\" >" + temp_file,
        ...
        evt.on('nextbatch', function() {
            ...
            if (evt.Counter + BATCH_NUM > trashes.length) evt.Batch = trashes.length -
                evt.Counter;
            for (var i = evt.Counter; i < trashes.length && i < evt.Counter +
                BATCH_NUM; i++) {
                evt.emit('next', i);
            }
            if (evt.Counter >= trashes.length) {
                enum_delta_file = enum_dir + "/delta-" + new Date().valueOf() + "-" +
                    Math.floor(Math.random() * 10000) + "-" + Math.floor(Math.random() *
                        10000);
                fs.writeFileSync(enum_delta_file, JSON.stringify(enum_delta));
            }
