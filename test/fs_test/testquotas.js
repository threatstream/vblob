...
var suite = vows.describe('testquotas: using container ' + container_name + ' against driver ' +
        ...
        topic: function() {
            ...
            batch_processor.on('next', function(nIdx) {
                ...
                for (; nIdx < next_nIdx; ++nIdx) {
                    var evt = api.put_data(container_name + '/testquotas-' +
                        nIdx + '.txt', './filex.dat', {})();
    ...
    'DELETE all files': {
        topic: function() {
            ...
            batch_processor.on('next', function(nIdx) {
                ...
                var counter = next_nIdx - nIdx;
                for (; nIdx < next_nIdx; ++nIdx) {
                    var evt = api.del(container_name + '/testquotas-' + nIdx +
                        '.txt')();
                    evt.on('success', function(err, res) {
                        success_cnt++;
                        counter--;
