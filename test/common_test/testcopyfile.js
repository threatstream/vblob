...
var suite = vows.describe('testcopyfile: using container ' + container_name + ' against driver ' +
...
suite.addBatch({
    ...
    'REPLACE container/testcopyfile-2.txt': {
        topic: api.put(container_name + '/testcopyfile-2.txt', {
            'cache-control': 'cache',
            'content-disposition': 'attachment; filename=copying.txt',
            'content-encoding': 'none',
            'content-type': 'text',
            'expires': 'Thu, 01 Dec 2011 16:00:00 GMT',
            'x-amz-meta-hello': 'hello',
            'content-length': 0,
    ...
    'GET 1 container/testcopyfile-2.txt': {
        ...
        'should have correct meta info': function(err, res) {
            ...
            assert.equal(res.headers['content-disposition'],
                'attachment; filename=copying.txt');
    ...
    'COPY container/testcopyfile-2.txt if-unmodified-since past': {
        topic: api.put(container_name + '/testcopyfile-2.txt', {
            'content-length': 0,
            'x-amz-copy-source': container_name + '/testcopyfile-1.txt',
            'x-amz-copy-source-if-unmodified-since': new Date(new Date().valueOf() -
                500000).toUTCString()
        }),
    ...
    'COPY container/testcopyfile-2.txt if-modified-since future': {
        topic: api.put(container_name + '/testcopyfile-2.txt', {
            'content-length': 0,
            'x-amz-copy-source': container_name + '/testcopyfile-1.txt',
            'x-amz-copy-source-if-modified-since': new Date(new Date().valueOf() +
                500000).toUTCString()
        }),
        'should respond with a 200 code': assertStatus(200)
    },
