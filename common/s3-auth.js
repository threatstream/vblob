...
var validate = function(keyID, secretID, method, targets, headers, signature) {
    ...
    var Authorization = auth.authorization({
        ...
        date: date,
        resource: auth.canonicalizeResource((targets.container === undefined || targets
            .container === null) ? '/' : (targets.filename ? /* join('/', targets.container, targets.filename)*/
            '/' + targets.container + '/' + targets.filename + utils.to_query_string(
                targets.query) : join('/', targets.container) + utils.to_query_string(
                targets.query))),
        contentType: content_type,
...
module.exports.authenticate = function(creds, method, targets, headers, signature, resp) {
    ...
    if (!key || !creds[key] || validate(key, creds[key], method, targets, headers, signature) ===
        false) {
        resp.resp_code = 401;
        resp.resp_header = {};
        resp.resp_body = {
